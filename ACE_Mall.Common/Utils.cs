    ...
    public class Utils
    {
        ...
        public static string SetimgUrl(string imgurl)
        {
            if (imgurl != null && imgurl != "")
            {
                ...
                {
                    imgurl = imgurl.Replace("/upload/", "http://121.40.186.118:9010/upload/");
                }
            }
            return imgurl;
        }
        ...
        #region  返回字符串前XX位字符
        public static string GetStartNumber(string str, int num)
        {
        ...
        #region  返回字符串后XX位字符
        public static string GetEndNumber(string str, int num)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetRamStr(int num)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                user_no += character[rnd.Next(character.Length)];
            }
            return user_no;
        }
        #endregion
        ...
        /// 替换读取多张缩略图
        /// </summary>
        /// <param name="img_url">图片路径</param>
        /// <param name="num">2..n</param>
        /// <returns></returns>
        public static string GetThumbImg(object img_url, int num)
        {
            return img_url.ToString().Replace("thumb_1_", "thumb_" + num + "_");
        }
        #endregion
        ...
        /// <returns></returns>
        public static string GetTimeStr(object _time)
        {
            return GetTimeStr(_time, "yyyy-MM-dd");
        }
        /// <summary>
        /// 统一时间格式调用
        /// </summary>
        /// <param name="_time">时间</param>
        /// <param name="TimeStr">时间格式</param>
        /// <returns></returns>
        public static string GetTimeStr(object _time, string TimeStr)
        {
            if (_time == null)
            {
                return "-";
            }
            if (string.IsNullOrEmpty(_time.ToString()))
            {
                return "-";
            }
            DateTime time = Utils.ObjectToDateTime(_time);
            TimeSpan tstime = DateTime.Now - time;
            double mtime = tstime.TotalMinutes;
            //一个小时60分钟
            if (mtime < 1)
            {
                return "刚刚";
            ...
            if (mtime < 60)
            {
                return mtime.ToString("0") + "分钟前";
            }
            //一天1440分钟
            else if (mtime < 1440)
            {
                return (mtime / 60).ToString("0") + "小时前";
            }
            //一个月43200分钟
            else if (mtime < 43200)
            {
                return (mtime / 1440).ToString("0") + "天前";
            }
            //超过一个月返回准确日期
            else if (mtime >= 43200)
            {
                return mtime.ToString(TimeStr);
            }
            return "-";
        }
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetRamIdcode()
        {
            string idcode = System.Guid.NewGuid().ToString();
            idcode = idcode.Replace("-", "");
            idcode = idcode.ToLower();
            return idcode;
        }
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetWebQQurl(string qq)
        {
        ...
        public static string Number(int Length, bool Sleep)
        {
            ...
            //    aaa += BitConverter.ToString(byteCsp);
            //}
            //return aaa;
        }
        /// <summary>
        ...
        public static string GetOrderNumber()
        {
            ...
            return num + Number(5, true).ToString();
        }
        ...
        /// <param name="strName">名称</param>
        /// <param name="strValue">值</param>
        /// <param name="strValue">过期时间(秒)</param>
        ...
        public static string OutPageListUL(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize)
        {
            ...
            if (pageIndex <= 1)
            {
                firstBtn = "<li class=\"prev\" ><a>上一页</a></li>";
            }
            if (pageIndex >= pageCount)
            {
                lastBtn = "<li class=\"next\"><a>下一页</a></li>";
            }
            if (pageIndex == 1)
            {
                firstStr = "<li  class=\"current\"><a  >1</a></li>";
            }
            if (pageIndex == pageCount)
            {
                lastStr = "<li  class=\"current\"><a  >" + pageCount.ToString() + "</a></li>";
            }
            int firstNum = pageIndex - (centSize / 2); //中间开始的页码
            if (pageIndex < centSize)
                firstNum = 2;
            ...
            for (int i = firstNum; i <= lastNum; i++)
            {
                if (i == pageIndex)
                {
                    pageStr.Append("<li class=\"current\"   ><a >" + i + "</a></li>");
                }
                else
                {
        ...
        #region 审核状态整数转字符串
        public static string AduitStat(int is_status)
        {
            string str = string.Empty;
            switch (is_status)
            {
                case 1: str = "<span style='color:#C0C0C0;'>未审核</span>"; break;
                case 2: str = "<span style='color:blue;'>通过</span>"; break;
                case 3: str = "<span style='color:red;'>未通过</span>"; break;
